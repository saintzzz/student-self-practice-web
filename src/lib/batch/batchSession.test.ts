...
import { getCurrentQuestion, submitExtraLetterAnswer, submitListeningAnswer } from '../practiceSession';
...
function answerCurrentQuestion(state: BatchState): BatchState {
  ...
  throw new Error(`Unexpected question kind for Round 1/2 in this build: ${question.kind}`);
}
...
describe('full Batch flow (AC17, AC21)', () => {
  it('moves through Round 1 -> Round 2 -> Round 3 stub -> Round 4 stub -> Batch summary', () => {
    ...
    expect(batch.completedRounds[2]?.implemented).toBe(false);
    expect(batch.completedRounds[2]?.totalCount).toBe(0);
    ...
    expect(result.totalQuestions).toBe(ROUND_1_QUESTION_COUNT + ROUND_2_QUESTION_COUNT);
