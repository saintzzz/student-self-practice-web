 ...
 * (Round 3/4 in this build) omits it entirely, and batchSession.ts /
 * BatchScreen.tsx render a stub placeholder instead of a question loop.
 *
 * To light up Round 3 or Round 4 in a follow-up task: build its generator
 * under src/lib/generators/, build its Round-level "pick ~10 from the pool"
 * function under src/lib/rounds/ (same shape as round1ExtraLetter.ts /
 * round2ListeningSentence.ts), then add `buildQuestions` to that round's
 * entry in roundDefinitions.ts. No changes to batchSession.ts, BatchScreen.tsx,
 * or the Batch state machine are needed for that step alone.
 */
